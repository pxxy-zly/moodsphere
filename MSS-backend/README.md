...
- `mood-asset`：图片/音频素材上传、存储、元数据管理
- `mood-ai-analyze`：多模态 AI 分析、关键词提取、风险语义识别
- `mood-vector`：情绪向量归一化、平滑、置信度计算
- `mood-weather-engine`：情绪天气映射规则、快照生成、视觉参数输出
- `mood-report`：日报/周报/月报、趋势统计、摘要生成
- `mood-globe`：匿名投影、区域聚合、时序流数据输出
- `mood-risk`：风险分级、预警记录、审核标记
- `mood-notify`：提醒与通知通道管理
- `mood-config`：标签、映射规则、提示词模板等配置中心
- `mood-stat`：运营统计、模型调用监控、成功率与成本分析
